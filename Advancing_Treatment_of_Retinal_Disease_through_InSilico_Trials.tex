...
Another difference with the previous work is the assumed capacity of the venules and central retinal vessels to dilate or collapse passively in response to pressure gradients across their walls.
...
Therefore, the use of artificial vasculatures with similar topological characteristics as the real one may be useful to model the whole extent of the retinal vasculature.
...
The effects of haemodynamics parameters on different parts of the tissue and the vasculature were analysed with the model and showed, among other things, the sensitivity of perfusion of retinal ganglion cells to blood viscosity and metabolic consumption~\cite{Causin_2015}.
...
Malek et al. tried to characterise the flow distribution in reproduced arterioles and veins to quantify the impact of vessel tortuosity on haemodynamic parameters~\cite{Malek_2014,Malek_2015}.
...
Reviews of some of these models can be found in other reviews~\cite{Arciero_2019,Roberts_2016}.
...
The lumped parameter models developed in this paper simulate haemodynamics between the CRA to the CRV in five vascular compartments~\cite{Chiaravalli_2021}.
...
In contact with the retina is the CC, the innermost layer of the choroid.
...
Since then, only Zouache et al. have modelled the physiology of the choroid and its peculiar architecture~\cite{Zouache_2015}.
...
This environment allowed haemodynamics simulations of individual patients characterised by a handful of parameters, yielding significantly different results for each combination of parameters.
...
This allowed separate investigation of red blood cell and platelet flows and showed the differences in penetration through the microaneurysm between the two cell types.
...
It is thought that the procedure treats retinal ischaemia by reducing the oxygen consumption of the photoreceptors affected by the laser-induced burn~\cite{Fawzi_2019,Gast_2016}.
...
The model predictions are along the lines of the seemingly increased flow in macular capillaries~\cite{Fawzi_2019}.
...
The model showed how different patterns of burn affect the propagation of ischaemia and suggested that targeting the non-ischaemic peripheral retina with appropriate patterns may be effective at containing it.
...
Knowledge of the total exposure of the retina and the CC to the drug is important to develop better therapeutic molecules and administration strategies and reduce risk and burden on the patient.
...
Analytical relationships between a molecule's characteristics and its ocular half-life can be derived from those models and can help the design of longer lasting therapeutics.
...
This work also highlighted the difference between \textit{in vitro} and \textit{in vivo} binding rates of anti-VEGF to VEGF, which could be of multiple orders of magnitude compared to values used in previous similar PK models~\cite{Saunders_2015}.
...
A similarly simple approach is to model each layer as a separate 0D compartment, predicting the evolving oxygen concentration in each compartment using time-dependent ODEs (\citep[see][]{German_et_al_2021}) who also model nitric oxide concentrations.
